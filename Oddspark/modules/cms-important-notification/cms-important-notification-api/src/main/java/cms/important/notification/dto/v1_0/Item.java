 ...
 */
@Generated("")
@GraphQLName(description = "Important notification item", value = "Item")
...
public class Item implements Serializable {
	...
	public DisplayPage[] getDisplayPages() {
		if (_displayPageMultiSupplier != null) {
			displayPageMulti = _displayPageMultiSupplier.get();

			_displayPageMultiSupplier = null;
		}
	...
	public void setDisplayPages(
		...
		_displayPageMultiSupplier = () -> {
			try {
				return displayPageMultiUnsafeSupplier.get();
			}
			catch (RuntimeException runtimeException) {
				throw runtimeException;
			}
	...
	@JsonIgnore
	private Supplier<DisplayPage[]> _displayPageMultiSupplier;
	...
	public String toString() {
		...
		DisplayPage[] displayPageMulti = getDisplayPages();

		if (displayPageMulti != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0; i < displayPageMulti.length; i++) {
				sb.append(String.valueOf(displayPageMulti[i]));

				if ((i + 1) < displayPageMulti.length) {
					sb.append(", ");
				}
			}
	...
	@io.swagger.v3.oas.annotations.media.Schema(
		...
		defaultValue = "cms.important.notification.dto.v1_0.Item",
		name = "x-class-name"
	)
...
// LIFERAY-REST-BUILDER-HASH:-1407606168